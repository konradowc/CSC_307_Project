...
const mockSavedPost = {
  ...
  test('submits a new post from CreatePost', async () => {
    ...
    // Fill out form
    fireEvent.change(screen.getByPlaceholderText(/enter post title/i), {
      target: { value: 'Test Title' },
    });
    fireEvent.change(screen.getByPlaceholderText(/enter post content/i), {
      target: { value: 'Test Content' },
    });
  
    const file = new File(['dummy'], 'flowers.png', { type: 'image/png' });
    const fileInput = document.querySelector('input#image-upload');
...
test('displays post in Profile page', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByAltText(/blog post visual/i)).toHaveAttribute('src', flowersImage);
  ...
  test('shows image preview when a image file is selected', async () => {
    ...
    global.fetch = jest.fn()
      .mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ user: { _id: '123', city: 'a' } }),
      })
      .mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({
  ...
  test('shows alert when image upload fails', async () => {
    ...
    const file = new File(['fail'], 'broken.png', { type: 'image/png' });
    const input = document.querySelector('input#image-upload');
    fireEvent.change(input, { target: { files: [file] } });
  
    await waitFor(() => {
      expect(alertSpy).toHaveBeenCalledWith('Image upload failed.');
    });
  ...
  test('shows alert when blog post submission fails', async () => {
    ...
    const file = new File(['real'], 'ok.png', { type: 'image/png' });
    fireEvent.change(document.querySelector('input#image-upload'), {
      target: { files: [file] },
    });
    ...
    await waitFor(() => {
      expect(alertSpy).toHaveBeenCalledWith('Could not save your post.');
    });
  ...
  test('shows error when deleting post fails', async () => {
    ...
    global.fetch = jest.fn().mockResolvedValueOnce({
      ok: false,
      json: () => Promise.resolve({ error: 'Mock deletion error' }),
    });
